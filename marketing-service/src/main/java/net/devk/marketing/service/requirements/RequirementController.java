...
import net.devk.marketing.service.requirements.dto.CreateNewSupplyRequirementRequestDTO;
...
public class RequirementController {
    ...
    @RequestMapping(path = "/{id}/supply", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> supplyRequirement(
            @RequestBody CreateNewSupplyRequirementRequestDTO createNewSupplyRequirementRequestDTO,
            @PathVariable(name = "id", required = true) Long requirementId) {

        requirementService.createSupplyRequirement(requirementId,
                createNewSupplyRequirementRequestDTO.getValue(),
                createNewSupplyRequirementRequestDTO.getSupplyStatusType());
