...
interface CustomerDocumentRepository extends JpaRepository<CustomerDocument, Long> {
	...
	@Query("select new net.devk.marketing.service.documents.dto.CustomerDocumentListDTO(cd.id,cd.documentName,cd.filePath,dt.id,dt.type,cd.customer.id) from CustomerDocument cd inner join cd.documentType dt where cd.customer.id=?1")
