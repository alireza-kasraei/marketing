...
interface MeetingRepository extends JpaRepository<Meeting, Long> {
    ...
    @Query("select new net.devk.marketing.service.meetings.dto.CustomerMeetingListDTO(m.id,m.scheduleDate,m.subject,m.customer.id) from Meeting m where m.customer.id=?1 order by m.scheduleDate")
