...
interface TargetMemberRepository extends JpaRepository<TargetMember, Long> {
	...
	@Query("select new net.devk.marketing.service.targets.dto.PersonnelTargetsListResponseDTO(t.id,t.value,t.name,tm.id,tm.value,tm.name,s.name,s.id) from TargetMember tm inner join tm.target t inner join tm.teamMember tmm inner join tmm.personnel p inner join t.service s where p.username=?1")
